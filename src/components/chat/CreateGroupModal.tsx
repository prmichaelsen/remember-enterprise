/**
 * CreateGroupModal — modal form for creating a new private group conversation.
 * Collects name, description, and invited members.
 * Creator is automatically assigned OWNER_PRESET permissions (auth_level: 0).
 * Invited members receive MEMBER_PRESET permissions (auth_level: 5).
 */

import { useState, useCallback } from 'react'
import { Users, X, Search, Plus, Lock } from 'lucide-react'
...
export function CreateGroupModal({
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Create Private Group" isLoading={creating}>
      <div className="space-y-4">
        ...
        {/* Privacy notice */}
        <div className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs ${t.badgeInfo}`}>
          <Lock className="w-3.5 h-3.5 shrink-0" />
          <span>This group is private. Only invited members can see and participate.</span>
        </div>
        ...
        {selectedUsers.length > 0 && (
          <p className={`text-xs ${t.textMuted}`}>
            {selectedUsers.length} member{selectedUsers.length !== 1 ? 's' : ''} will be invited (+ you as owner)
          </p>
        )}
